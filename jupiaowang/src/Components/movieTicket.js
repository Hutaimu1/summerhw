...
class movieTicket extends React.Component {
    ...
    getMovie = () => {
        let result = [];
        const {data, current, pageSize, chooseMovie, chooseDate, chooseBrand, chooseTime, date, brand, time, price, place} = this.state;
            ...
            if (index >= (current - 1) * pageSize && index < current * pageSize) {
                result.push(<Card
                    ...
                    actions={[<Icon type="eye"
                                    onClick={() => this.props.history.push('/home/' + this.props.match.params.userName + "/viewMovie/" + card.id)}/>,
                        <Popover
                            placement="rightTop"
                            content={<div style={{overflow: "hidden"}}>
                                <div className="tags-panel container">
                                    <ul className="tags-lines">
                                        ...
                                        <li className="tags-line tags-line-border"
                                            style={{display: [chooseDate === '' || chooseBrand === '' ? "none" : "block"]}}>
                                ...
                                <Button type="primary" className="tags-button"
                                        disabled={chooseDate === "" || chooseBrand === "" || chooseTime === ""}
                                        onClick={() => this.movieTicketAddToShopCart(card.id, place, chooseDate, chooseBrand, chooseTime, price)}>加入购物车￥{price}</Button>
                            </div>}
                            trigger="click"
                            visible={chooseMovie === card.title ? this.state.shopVisible : false}
                        ...
                        , <Popover
                            ...
                            content={<div style={{overflow: "hidden"}}>
                                ...
                                <Button type="primary" className="tags-button"
                                        disabled={chooseDate === "" || chooseBrand === "" || chooseTime === ""}
                                        onClick={() => this.movieTicketQuickBuy(card.id, place, chooseDate, chooseBrand, chooseTime, price)}>一键快速下单￥{price}</Button>
                            </div>}
                            trigger="click"
                ...
                >
                    <Card.Meta
                        ...
                        description={<span><Rate allowHalf defaultValue={card.rate} disabled/><span
                            style={{color: "#ff5741"}}>评分:{card.rate * 2}</span></span>}
                    />
                </Card>)
            }
        });
            ...
            <div style={{fontWeight: "bold", overflow: "hidden"}}>
                ...
                <Popover
                    ...
                    title={<div className="tab-title">
                        <span style={{fontWeight: "bold"}} className={this.state.chooseTag === "热门" ? "on" : ""}
                              onClick={() => this.changeTag("热门")}>热门</span>
                        <span className={this.state.chooseTag === "A - G" ? "on" : ""}
                              onClick={() => this.changeTag("A - G")}>A - G</span>
                        <span className={this.state.chooseTag === "H - L" ? "on" : ""}
                              onClick={() => this.changeTag("H - L")}>H - L</span>
                        <span className={this.state.chooseTag === "M - T" ? "on" : ""}
                              onClick={() => this.changeTag("M - T")}>M - T</span>
                        <span className={this.state.chooseTag === "W - Z" ? "on" : ""}
                              onClick={() => this.changeTag("W - Z")}>W - Z</span>
                    </div>}
                    content={<div className="tab-content" style={{fontWeight: "bold"}}>
                        <div style={{display: this.state.chooseTag === "热门" ? "block" : "none"}}>
                            {hotCity.map(city => <span key={city}><a
                                onClick={() => this.changeCity(city)}>{city}</a></span>)}
                        </div>
                        <div style={{display: this.state.chooseTag === "A - G" ? "block" : "none"}}>
                            {CityA_G.map(index => <dl className="tab-content-mod" key={index[0]}>
                                <dt>{index[0]}</dt>
                                <dd>
                                    {index[1].map(city => <span key={city}><a
                                        onClick={() => this.changeCity(city)}>{city}</a></span>)}
                                </dd>
                            </dl>)}
                        </div>
