...
let myMessage = {userName: "longer150721",password:"htm123280520",content: ["275868482@qq.com", "13305806767", "275868482"],};
...
class user extends React.Component{
    ...
    handleModify = () => {
        ...
        result.content = [email,phone,QQ];
        if(password === userMessage.password && email === userMessage.content[0] && phone === userMessage.content[1] && QQ === userMessage.content[2]) {
    ...
    enableClick = () =>{
        ...
        else if(email !== this.state.userMessage.content[0] &&(!this.haveValue(captcha)|| (this.haveValue(captcha) && (captcha!==this.state.validate)))){
            return true;
        }
        return false;
    };
                    ...
                    <Form>
                        ...
                        >
                            {getFieldDecorator('password', {
                                ...
                                isEdit?<Input  type="password"/>:<span>{user.repeat('*',userMessage.password.length)}</span>
                            )}
                        </Form.Item>
                        {isEdit?
                            <Form.Item
                        ...
                        {isEdit?<Form.Item
                            ...
                            {getFieldDecorator('captcha', {
                                rules: [ {required:this.props.form.getFieldValue('email') !== userMessage.content[0] && this.haveValue(this.props.form.getFieldValue('email')),
                                message:'验证码不能为空'},{
                                    validator: this.checkCaptacha
                                }],
                            })(
                                <Input className="registration-form-input-captcha" placeholder="请输入验证码" disabled={this.props.form.getFieldValue('email') === userMessage.content[0] || !this.haveValue(this.props.form.getFieldValue('email'))}/>
                            )}
                            <Button type="primary"
                                    onClick={this.renderCode}
                                    className="registration-form-button-captcha"
                                    disabled={this.props.form.getFieldValue('email') === userMessage.content[0] || !this.haveValue(this.props.form.getFieldValue('email'))}>
