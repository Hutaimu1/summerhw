...
public class TicketOrderServiceImpl implements TicketOrderService {
    ...
    @Override
    public  String orderOutOfTime(int orderId) {
        Gson gson = new Gson();
        TicketOrderEntity ticketOrder = ticketOrderRepository.findByOrderId(orderId).get(0);
        ticketOrder.setIsPaid((byte) 2);
        ticketOrderRepository.save(ticketOrder);
        List<OrderItemEntity> orderItem = orderItemRepository.findByOrderId(orderId);
        for (OrderItemEntity oneOrder : orderItem) {
            int id = oneOrder.getShopcartId();
            byte type = shopCartRepository.findByShopcartId(id).get(0).getType();
            if (type == 0) {
                int count = shopCartRepository.findByShopcartId(id).get(0).getCount();
                List<TrainTicketEntity> ticket = trainTicketRepository.findByTicketId(id);
                if (ticket.size() != 0) {
                    TrainTicketEntity oneShopCart = ticket.get(0);
                    oneShopCart.setLeftTicket(oneShopCart.getLeftTicket() + count);
                    trainTicketRepository.save(oneShopCart);
                }
            }
