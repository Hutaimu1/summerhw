...
import {Card, Icon, Rate, Divider, Pagination, Popover, Button,Popconfirm} from 'antd';
import $ from "jquery";
...
class movieTicket extends React.Component {
    ...
    addToCollection = (id) =>{
        let userName = this.props.match.params.userName;
        $.ajax({
            url: "bookstoreApp/movieTicketAddToCollection",
            data: {ticketId:id,userName:userName},
            type: "POST",
            traditional: true,
            success: function (data) {
                if(JSON.parse(data) === true){
                    message.success("票品添加到收藏夹成功，请前往我的收藏中查看.");

                }
                else{
                    message.warning("您已经收藏过该票品!")
                }
            }
        });
    };
            ...
            if (index >= (this.state.current - 1) * this.state.pageSize && index < this.state.current * this.state.pageSize) {
                result.push(<Card
                    ...
                    actions={[<Icon type="eye" onClick={() => this.props.history.push('/home/' + this.props.match.params.userName + "/viewMovie/" + card.id)} />,
                        <Popconfirm title={<div style={{width:"150px"}}>您确定要将该票品加入到收藏吗?</div>} onConfirm={() => this.addToCollection(card.id)}>
                            <Icon type="heart"/>
                        </Popconfirm>,
                        <Popover
                            placement="rightTop"
